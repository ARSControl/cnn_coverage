...
path = Path("/content/drive/MyDrive/Colab Notebooks/coverage/logs/gmm_coverage")
...
plt.scatter(points[id, 0], points[id, 1], c='r')

"""## Generate local grid (sensing range)"""
...
plot_occgrid(Xg_i, Yg_i, Z_i)

neighs = np.delete(points, id, 0)
...
plt.scatter(local_pts[:, 0], local_pts[:, 1])
plt.xlim([-ROBOT_RANGE, ROBOT_RANGE])
plt.ylim([-ROBOT_RANGE, ROBOT_RANGE])
...
for i in range(GRID_STEPS):
  for j in range(GRID_STEPS):
    # jj = GRID_STEPS-1-j
    p_ij = np.array([-ROBOT_RANGE+j*step, -ROBOT_RANGE+i*step])
...
fig, ax = plt.subplots(1, 1, figsize=(6,6))
plot_occgrid(Xg_i, Yg_i, img_i, ax=ax)
ax.scatter(local_pts[:, 0], local_pts[:, 1])
ax.set_xlim([-ROBOT_RANGE, ROBOT_RANGE])
ax.set_ylim([-ROBOT_RANGE, ROBOT_RANGE])

img_i[-1, 0]

# imgs = np.zeros((NUM_STEPS, ROBOTS_NUM, GRID_STEPS, GRID_STEPS))
...
for s in range(1, NUM_STEPS+1):
  ...
  for idx in range(ROBOTS_NUM):
    ...
    img_s[idx, :, :] = img_i
    fig, ax = plt.subplots(1, 1, figsize=(6,6))
    plot_occgrid(Xg, Yg, img_i, ax=ax)
    ax.scatter(local_pts[:, 0], local_pts[:, 1])
    # ax.set_xlim([-ROBOT_RANGE, ROBOT_RANGE])
    # ax.set_ylim([-ROBOT_RANGE, ROBOT_RANGE])
...
plot_occgrid(Xg_i, Yg_i, imgs[11, 1, :, :])
