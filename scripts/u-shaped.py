...
for episode in range(EPISODES):
  ...
  ax.plot([obstacles[0]-0.5*OBS_W, obstacles[0]+0.5*OBS_W], [obstacles[1]-0.5*OBS_H, obstacles[1]-0.5*OBS_H], c='tab:blue')
  ax.plot([obstacles[0]+0.5*OBS_W, obstacles[0]+0.5*OBS_W], [obstacles[1]-0.5*OBS_H, obstacles[1]+0.5*OBS_H], c='tab:blue')
  ax.plot([obstacles[0]+0.5*OBS_W, obstacles[0]-0.5*OBS_W], [obstacles[1]+0.5*OBS_H, obstacles[1]+0.5*OBS_H], c='tab:blue')
  ax.plot([obstacles[0]-0.5*OBS_W, obstacles[0]-0.5*OBS_W], [obstacles[1]+0.5*OBS_H, obstacles[1]-0.5*OBS_H], c='tab:blue')
  # ---------- Simulate episode ---------
  ...
  points[:, 0] = -0.5*AREA_W + 6 * np.random.rand(ROBOTS_NUM)
    ...
    for idx in range(ROBOTS_NUM):
      ...
      for i in range(GRID_STEPS):
        for j in range(GRID_STEPS):
          ...
          # Check for obstacles
          if p_w[0] > obstacles[0] - 0.6*OBS_W and p_w[0] < obstacles[0] + 0.6*OBS_W and p_w[1] > obstacles[1] - 0.6*OBS_H and p_w[1] < obstacles[1] + 0.6*OBS_H:
      ...
      if p_i[0] > obstacles[0] - 0.5*OBS_W and p_i[0] < obstacles[0] + 0.5*OBS_W and p_i[1] > obstacles[1] - 0.5*OBS_H and p_i[1] < obstacles[1] + 0.5*OBS_H:
